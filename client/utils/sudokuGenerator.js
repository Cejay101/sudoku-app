...
function countSolutions(board, size) {
  ...
  function solve() {
    ...
    outer: for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        if (board[r][c] === 0) { emptyRow = r; emptyCol = c; break outer; }
      }
    }
    if (emptyRow === -1) { count++; return count >= 2; }
...
export function generatePuzzle(size, difficulty) {
  ...
  for (const [r, c] of positions) {
    ...
    if (countSolutions(boardCopy, size) === 1) { removed++; }
    else { puzzle[r][c] = backup; }
  }

  return { puzzle: puzzle.map((row) => [...row]), solution: solved, size, difficulty };
}
